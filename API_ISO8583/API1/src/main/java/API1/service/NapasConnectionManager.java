...
public class NapasConnectionManager {
    ...
    // ZaloPay service URL => port 1111
    private static final String ZALO_HOST = "http://localhost:1111";
    ...
    private void readLoop() {
        while(!socket.isClosed()){
            try {
                ...
                if(fut!=null){
                    // response for sendAndWait
                    fut.complete(iso);
                } else {
                    // inbound request => handle
                    System.out.println("[API1 NapasConn] inbound request => handleInbound");
    ...
    private ISOMsg buildIsoResponse(ISOMsg req, Map<String,String> j) throws ISOException {
        // copy fields from req => 0210 => set F39 from j
        ISOMsg resp= buildResponse(req, j.getOrDefault("F39","96"));
